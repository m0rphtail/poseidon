...
pub fn run(domain: &str, output: &str) {
    let tmp_output = [output, "tmp"].join(".");
    println!("{}", tmp_output);
    get_subdomains(domain, &tmp_output);
}

fn get_subdomains(domain: &str, output: &str) {
    let sp = Spinner::new(&Spinners::Earth, "hunting subdomains".into());
...
fn resolve_subdomains(output: &str) {
    ...
    let massdns = Command::new("massdns").args(&["-r","resolvers.txt","-t","A","o","S"]);
}

// let RESOLVERS: Vec<&str> = vec![
//             "103.20.188.35",
//             "103.20.188.83 ",
//             "103.22.248.62",
//             "103.3.46.105",
